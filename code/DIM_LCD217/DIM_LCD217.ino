...
/*
 le bouton "reset" sur l'arduino est inaccessible, mais il y en a un sur le LCD qui fonctionne
  ...
  le bouton SELLECT est utilisé pour débuter le programe. 
  ...
  le bouton LEFT débute le test 2, qui consiste à montrer les signaux de la tour

  Lorsqu'un programme est actif, la lumière du LCD s'éteint. pour la rallumer, il faut tout simplement peser le bouton DOWN

  Lorsqu'un programme est actif,le LCD montre le nom du programme et le temps depuis le début du programme. 
  Vous pouvez aussi peser RIGHT pour voir la variable PWM (en pourcentage) et les signaux de la tour
*/
...
void setup()
{
  ...
  lapberPages = sizeof(program)/sizeof(program[1]); //the lapber of pages is equal to the size of the "program" array.
...
void action(){// function to determine what to do when a button is pressed
     switch (read_LCD_buttons())
 {
   case btnRIGHT:
     {
      int temp = currentPage;//store the page lapber, so we don't loose it
      currentPage = 99; //set current page to 99 (TEST1)
      lcd.clear();
      Select();//start the program
      currentPage = temp;//re set the page lapber
     break;
     }
   case btnLEFT:
     {
   ...
   case btnDOWN:
     {
      scrollNo = 0;
      lcd.clear();
      if(currentPage + 1 < lapberPages){//go to the next page if it exists
        currentPage++;
      }
     break;
     }
   case btnSELECT:
     {
     lcd.clear();
     Select();//start the program with the current page lapber
     break;
     }
